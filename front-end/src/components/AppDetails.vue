<template>
  <div class="scene -detail" ref="details" :style="{ top: topAmt + 'px' }">
    <div class="detail">
      <div class="content">
        <h2 class="title" ref="title">{{ currentItem.shop_name }}</h2>
        <div class="tags">
          <span>Price: {{ marker.price }}</span>
          <span>Original Price: {{ marker.ori_price }}</span>
        </div>
        <div class="tags">
          <span>Size: {{ marker.item_size }}</span>
          <span>Quantity: {{ marker.pack_ll }} to {{ marker.pack_ul }}</span>
          <span>Distance: 1.3KM</span>
        </div>
        <p class="description" ref="desc">Deadline: {{ marker.end_time }}</p>
      </div>
    </div>
  </div>
</template>
